...
* ~~Have a look around the files here. Where are the R scripts? What are the directories `data` and `figs` for?~~
    - R scripts are located in the `R` folder of the *packages-report* directory. The directories `data` and `figs` contain the .csv data files and any output figures, respectively.
...
The goal of packages-report is to TAKE YOU TO THE ABSOULTE BRINK OF EDGELORD-HOOD

```{r, include = FALSE}
## load data/add-on-packages-freqtable.csv here in this chunk
```

I have `all teh` add-on packages installed.
